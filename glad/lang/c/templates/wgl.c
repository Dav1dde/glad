...
int gladLoad{{ feature_set.api|upper }}Simple(HDC hdc, void* (*)(const char *name, void* userptr) load) {
    return gladLoad{{ feature_set.api|upper }}(hdc, screen, glad_get_proc_from_userptr, &load);
}
{% endblock %}
